...
const SideBar = ({selectedUser, setselecteUser}) => {
    ...
    <div className={`bg-[#8185b2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white 
      ...
      <div className='flex flex-col'>
        {userDummyData.map((user, index) =>(
            <div onClick={()=> {setselecteUser(user)}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm: text-sm 
                           ${selectedUser?._id === user._id && 'bg-[#28142]/50'} `}>
                ...
                {index > 2 && <p className='absolute top-4 right-4 text-xs h-5 w-5 bg-violet-500/50 flex justify-center items-center rounded-full'>{index}</p>}
            </div>
        ))}
      </div>
    </div>
