...
const ChatContainer = ({selectedUser, setselecteUser}) => {
    const scrollEnd = useRef();

    useEffect(() => {
        if(scrollEnd.current) {
            scrollEnd.current.scrollIntoView({behaviour: "smooth"}); // smooothly scroll the page to the scrollend ref div
        }
    }, [])
    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6 '>
        {messagesDummyData.map((msg, index) => {
           return <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId != '680f5116f10f3cd28382ed02' && 'flex-row-reverse'} `}>
                {
                    msg.image?(
                    ...
                    (
                        <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all bg-violet-500/30 text-white
                            ${msg.senderId === '680f5116f10f3cd28382ed02' ? 'rounded-br-none' : 'rounded-bl-none'} `}>{msg.text}
                        </p>
                    )
                }
        ...
        {/* bottom areal */}
        <div className='absolute botton-0 left-0 right-0 flex items-center gap-3 p-3'>
